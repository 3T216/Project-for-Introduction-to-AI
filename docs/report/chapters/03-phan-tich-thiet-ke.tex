...
- \textbf{Module chính:}
  \begin{itemize}
    \item \textbf{service.py}: Các hàm higher-level như \texttt{find\_routes()}, \texttt{find\_nearest\_station()}, filter graph.
  ...
  \end{itemize}
- \textbf{Endpoints API:} \texttt{/api/network}, \texttt{/api/routes}, \texttt{/api/nearest-station}, \texttt{/api/routes-by-points}.
...
- \textbf{Dữ liệu chính:}
  ...
  \end{itemize}
- \textbf{Nguồn:} OpenStreetMap (file \texttt{data/shanghai\_metro\_osm.json}, 206 KB).
...
Đồ thị được lưu: \texttt{graph: dict[str, list[Edge]]}

Ví dụ: \texttt{graph[\textquotedbl Hongqiao Railway Station\textquotedbl] = [Edge(\textquotedbl Songhong Road\textquotedbl, 6.0, \textquotedbl Line 2\textquotedbl, 4.8), ...]}
...
\begin{enumerate}
  ...
  \item Gửi HTTP GET request tới \texttt{/api/routes-by-points} hoặc \texttt{/api/routes}.
  \item Server.py nhận request, parse parameters.
  \item Gọi \texttt{service.find\_routes()} với các tham số.
  \item Service tạo graph đã lọc (loại bỏ tuyến/đoạn bị cấm).
  \item Service gọi hàm thuật toán tương ứng (UCS/Greedy/A*/Dijkstra).
  \item Thuật toán trả lại \texttt{SearchResult} (path, cost, explored\_count, line\_sequence, distance).
