...
│   └── styles.css
├── data/
│   └── shanghai_metro_osm.json     # 411 ga, 20 tuyến (206 KB)
├── tests/
│   ├── test_algorithms.py
│   └── test_http_server.py
└── README.md
...
\begin{itemize}
  \item \textbf{4 endpoints × (happy path + malformed input):} Test `/api/network`, `/api/routes`, `/api/nearest-station`, `/api/routes-by-points`.
...
\begin{itemize}
  ...
  \item Một số ga tên chỉ là số (ví dụ "1", "2") do data OSM thiếu tag `name`.
\end{itemize}
